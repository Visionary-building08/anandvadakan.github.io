  ...
  useEffect(() => {
    ...
    class Program {
      constructor(vertexShader, fragmentShader) {
        this.uniforms = {}; this.program = createProgram(vertexShader, fragmentShader)
    ...
    function correctRadius(radius) { const ar = canvas.width / canvas.height; if (ar > 1) radius *= ar; return radius }
    ...
    function correctDeltaX(delta) { const ar = canvas.width / canvas.height; if (ar < 1) delta *= ar; return delta }
    function correctDeltaY(delta) { const ar = canvas.width / canvas.height; if (ar > 1) delta /= ar; return delta }

    function hexToRGB(hex) {
      let val = hex.replace('#', '')
      if (val.length === 3) val = val[0]+val[0]+val[1]+val[1]+val[2]+val[2]
      return { r: parseInt(val.slice(0,2),16)/255*0.15, g: parseInt(val.slice(2,4),16)/255*0.15, b: parseInt(val.slice(4,6),16)/255*0.15 }
    }

    function generateColor() {
      if (!config.RAINBOW_MODE) return hexToRGB(config.COLOR)
      const c = HSVtoRGB(Math.random(), 1, 1); c.r *= 0.15; c.g *= 0.15; c.b *= 0.15; return c
    }

    function HSVtoRGB(h, s, v) {
      let r, g, b; const i = Math.floor(h*6), f = h*6-i, p = v*(1-s), q = v*(1-f*s), t = v*(1-(1-f)*s)
      switch (i%6) { case 0: r=v;g=t;b=p;break; case 1: r=q;g=v;b=p;break; case 2: r=p;g=v;b=t;break; case 3: r=p;g=q;b=v;break; case 4: r=t;g=p;b=v;break; case 5: r=v;g=p;b=q;break; default: break }
      return { r, g, b }
    }
    ...
    function handleMouseMove(e) {
      const pointer = pointers[0], posX = scaleByPixelRatio(e.clientX), posY = scaleByPixelRatio(e.clientY)
      if (!firstMouseMoveHandled) { updatePointerMoveData(pointer, posX, posY, generateColor()); firstMouseMoveHandled = true }
      else updatePointerMoveData(pointer, posX, posY, pointer.color)
    }
    function handleTouchStart(e) {
      const pointer = pointers[0]
      for (let i=0;i<e.targetTouches.length;i++) updatePointerDownData(pointer, e.targetTouches[i].identifier, scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY))
    }
    function handleTouchMove(e) {
      const pointer = pointers[0]
      for (let i=0;i<e.targetTouches.length;i++) updatePointerMoveData(pointer, scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY), pointer.color)
    }
    function handleTouchEnd(e) { for (let i=0;i<e.changedTouches.length;i++) updatePointerUpData(pointers[0]) }
